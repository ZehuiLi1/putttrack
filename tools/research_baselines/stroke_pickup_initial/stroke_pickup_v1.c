...
static void finish(struct spv1_context *c) {
    ...
    } else if(impact && rotation && !c->held_hint) {
...
void spv1_push(struct spv1_context *c,const struct spv1_sample *s) {
    ...
    bool quiet=fabsf(an-c->gravity)<=SPV1_QUIET_ACCEL_DEVIATION_MPS2 && gn<=SPV1_QUIET_GYRO_RADS;
    ...
    if(c->window.active){
        ...
        if(da>=SPV1_CONTACT_DELTA_MPS2)c->last_contact_us=s->time_us;
...
const char *spv1_event_name(uint32_t t){
    switch(t){case SP_STROKE_LIKE:return "STROKE_LIKE_CANDIDATE";case SP_PICKUP_SUSPECTED:return "PICKUP_SUSPECTED";
    case SP_CONTACT_MOVING:return "CONTACT_WHILE_MOVING_UNRESOLVED";case SP_QUALITY_BREAK:return "QUALITY_BREAK";
