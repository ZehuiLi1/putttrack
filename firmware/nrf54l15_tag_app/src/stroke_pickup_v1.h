...
enum spv1_event_type {
    ...
    SP_ONSET_UNRESOLVED, SP_QUALITY_BREAK, SP_STROKE_PENDING
};
enum spv1_quality {
...
struct spv1_window {
    bool active, up_valid, stroke_checked, stroke_pending;
    uint64_t start_us;
...
struct spv1_context {
    ...
    float quiet_accel_sum[3], quiet_norm_mean, gravity, previous_accel[3], previous_gyro[3];
};
void spv1_init(struct spv1_context *c);
void spv1_invalidate(struct spv1_context *c);
/* Explicit operator bench boundary only, never inferred from IMU quietness. */
