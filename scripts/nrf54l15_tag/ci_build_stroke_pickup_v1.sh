...
OUT_DIR="${OUT_DIR:-$PWD/build/ci-nrf54l15-stroke-pickup-v1}"
...
APP_OVERLAY="$OTA_DIR/nrf54l15tag_ota.overlay;$OTA_DIR/nfc_service.overlay"
...
west build --sysbuild -p always \
  ...
  "-Dnrf54l15_tag_app_EXTRA_CONF_FILE=$OTA_DIR/stroke_pickup_v1_nfc.conf" \
...
grep -q '^CONFIG_PUTTTRACK_STROKE_PICKUP_V1=y$' "$APP_BUILD/zephyr/.config"
...
grep -E '^(CONFIG_PUTTTRACK_STROKE_PICKUP_V1|CONFIG_MCUBOOT_IMGTOOL_SIGN_VERSION)=' \
  "$APP_BUILD/zephyr/.config"
if [ -f "$APP_BUILD/zephyr/zephyr.map" ]; then
  grep -E 'spv1_(push|events|init)' \
