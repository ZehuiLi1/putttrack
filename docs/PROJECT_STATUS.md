...
a powered end-to-end NFC service path and a complete software-only one-hole
...
|---|---|---|---|
| nRF54L15 Tag firmware | Physical pass; recovery fault path pending | Confirmed `0.1.16`; NFC, healthy sensors, auto idle, SMP and post-confirm reset passed | Controlled R0 motion data; separately inject/reproduce sensor faults |
...
| NFC reader bench | Physical pass | ESP32-C3 + PN532 reads NFC-A/NTAG and strictly decodes the powered nRF54L15 URI; demand-loaded parser handles its 992 B advertised area | Cold-start and distance/orientation/near-metal sweep |
| NFC Tag/service | Powered physical pass | Confirmed `0.1.16`, 1.0 uH loop + provisional 220 pF pair, >50 strict URI reads, balanced field counters, bounded 10 s BLE window and return to idle | Instrumented tuning/range/current, then separate System OFF experiment |
...
| Sensor fault recovery | Healthy-path physical pass | A real assembled-ball ADXL367 boot-init fault was diagnosed; confirmed `0.1.16` retains healthy operation while adding retries, capture-generation invalidation, one guarded reboot and quarantine | Fault injection and ten sealed reset cycles |
...
1. Collect the roller and putter impact/roll/settle/stop episode matrix on
   confirmed `0.1.16`; the two-orientation stationary baseline is complete.
...
3. In parallel, characterize NFC cold-start, range/orientation and the
   provisional 1.0 uH plus 220 pF pair; do not call the close powered read final
   tuning or System OFF wake.
...
Powered NFC service identity and bounded handoff have passed. The research ball
...
- an instrumented NFC range/tuning/current or System OFF experiment;
