...
**分析日期：** 2026-09-04  
**主要数据包：** `putttrack_imu_dataset_20260904(1).zip`  
**额外检查：** `imu_data_20250306_161150.csv`
...
按整回合 leave-one-episode-out 比较：

| 方法 | Pickup F1 | 错误 |
|---|---:|---|
| 后验物理规则 | 1.000 | 0，但不是独立验证 |
...
`imu_data_20250306_161150.csv` 有 434 行、约 60.0 秒，中位有效采样率约 7.22 Hz，且没有动作标签、session、球体结构、固件和 clipping 元数据；其列和单位也与 2026 数据包不同。

因此它没有被混入本次模型比较。它可以保留为 legacy qualitative reference，但不能用于正式训练或准确率计算。
