...
:root{font-family:system-ui,sans-serif;background:#0b0d12;color:#f7f8fb}body{margin:0;min-height:100vh;display:grid;place-items:center}.shell{width:min(1100px,94vw);display:grid;gap:20px}.hero{padding:28px;border:1px solid #2a2f3a;border-radius:24px;background:#151923}.top{display:flex;justify-content:space-between;gap:16px}.hole{font-size:18px;opacity:.72}.title{font-size:48px;font-weight:800}.cue{display:flex;align-items:center;gap:18px;padding:22px;border-radius:20px;background:#202632}.lamp{width:58px;height:58px;border-radius:999px;background:#8b93a5}.lamp.green{background:#6fe59c;box-shadow:0 0 36px #6fe59c80}.lamp.amber{background:#ffbf52;box-shadow:0 0 36px #ffbf5280}.lamp.blue{background:#63a5ff;box-shadow:0 0 36px #63a5ff80}.status{font-size:42px;font-weight:750}.player{font-size:24px;opacity:.8}.grid{display:grid;grid-template-columns:1.4fr 1fr;gap:20px}.card{padding:24px;border-radius:20px;background:#151923;border:1px solid #2a2f3a}.ranking{display:grid;gap:10px}.row{display:flex;justify-content:space-between;padding:12px;background:#202632;border-radius:12px}.stroke-total{font-size:96px;font-weight:800;line-height:1.1;font-variant-numeric:tabular-nums}.stroke-panel{padding-bottom:24px}.hole-scores{display:grid;gap:8px;margin-top:18px}.toast{min-height:32px;font-size:22px;font-weight:650}.hint{opacity:.72}.dev button{margin:4px;padding:10px 14px;border:0;border-radius:10px}@media(max-width:760px){.grid{grid-template-columns:1fr}.title{font-size:36px}}</style></head><body><main class="shell"><section class="hero"><div class="top"><div><div id="hole" class="hole">HOLE</div><div id="title" class="title">PuttTrack</div></div><div id="instruction" class="hint"></div></div><div class="cue"><div id="lamp" class="lamp"></div><div><div id="status" class="status">AVAILABLE</div><div id="player" class="player">Present your assigned ball</div></div></div></section><section class="grid"><div class="card"><section class="stroke-panel" aria-label="Current hole strokes"><div class="hint">本洞杆数 · Strokes this hole</div><div id="stroke-count" class="stroke-total" role="status" aria-live="polite" aria-atomic="true">—</div><div id="stroke-owner" class="hint">等待球员就绪</div><div id="hole-scores" class="hole-scores"></div></section><div id="toast" class="toast"></div><div class="hint">No screen interaction is required during normal play.</div><details class="dev"><summary>Simulation controls</summary><div id="balls"></div><button id="stroke-btn">Stroke</button><button id="feature-btn">Precision +25</button><button id="cup-btn">Cup</button></details></div><div class="card"><h3>Leaderboard</h3><div id="ranking" class="ranking"></div></div></section></main><script>
let current=null;
...
function renderStrokes(){const ap=current.active_player;const states=current.player_hole_state||{};const active=ap?states[ap.player_id]:null;document.querySelector('#stroke-count').textContent=active?String(active.strokes):'—';document.querySelector('#stroke-owner').textContent=ap?ap.display_name+' · 已确认杆数':'等待球员就绪';const root=document.querySelector('#hole-scores');clear(root);for(const player of (current.ranking||[])){const hole=states[player.player_id];if(!hole)continue;const row=document.createElement('div');row.className='row';const name=document.createElement('span');name.textContent=player.display_name;const count=document.createElement('strong');count.textContent=String(hole.strokes)+' 杆';row.append(name,count);root.appendChild(row)}}
...
class VenueApplication:
    ...
    def observe_physical(self, payload: dict[str, Any]) -> dict[str, Any]:
        if self.runtime is None:
            raise CheckInError("check in first")
        observation = record_from_dict(payload)
        if not isinstance(observation, PhysicalSensorObservation):
            raise ValueError("record_type must be physical_sensor_observation")
        return self.runtime.process_physical_sensor_observation(observation).to_dict()
        ...
        def do_POST(self) -> None:
            try:
                ...
                if path == "/api/evidence/motion":
                    ...
                    return
                if path == "/api/evidence/physical":
                    decision = app.observe_physical(body)
                    self._send_json(
                        202,
                        {"ok": True, "decision": decision, "state": app.state()},
                    )
