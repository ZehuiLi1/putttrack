...
:root{font-family:system-ui,sans-serif;background:#0b0d12;color:#f7f8fb}body{margin:0;min-height:100vh;display:grid;place-items:center}.shell{width:min(1100px,94vw);display:grid;gap:20px}.hero{padding:28px;border:1px solid #2a2f3a;border-radius:24px;background:#151923}.top{display:flex;justify-content:space-between;gap:16px}.hole{font-size:18px;opacity:.72}.title{font-size:48px;font-weight:800}.cue{display:flex;align-items:center;gap:18px;padding:22px;border-radius:20px;background:#202632}.lamp{width:58px;height:58px;border-radius:999px;background:#8b93a5}.lamp.green{background:#6fe59c;box-shadow:0 0 36px #6fe59c80}.lamp.amber{background:#ffbf52;box-shadow:0 0 36px #ffbf5280}.lamp.blue{background:#63a5ff;box-shadow:0 0 36px #63a5ff80}.status{font-size:42px;font-weight:750}.player{font-size:24px;opacity:.8}.grid{display:grid;grid-template-columns:1.4fr 1fr;gap:20px}.card{padding:24px;border-radius:20px;background:#151923;border:1px solid #2a2f3a}.ranking{display:grid;gap:10px}.row{display:flex;justify-content:space-between;padding:12px;background:#202632;border-radius:12px}.toast{min-height:32px;font-size:22px;font-weight:650}.hint{opacity:.72}.dev button{margin:4px;padding:10px 14px;border:0;border-radius:10px}@media(max-width:760px){.grid{grid-template-columns:1fr}.title{font-size:36px}}</style></head><body><main class="shell"><section class="hero"><div class="top"><div><div id="hole" class="hole">HOLE</div><div id="title" class="title">PuttTrack</div></div><div id="instruction" class="hint"></div></div><div class="cue"><div id="lamp" class="lamp"></div><div><div id="status" class="status">AVAILABLE</div><div id="player" class="player">Present your assigned ball</div></div></div></section><section class="grid"><div class="card"><div id="toast" class="toast"></div><div class="hint">No screen interaction is required during normal play.</div><details class="dev"><summary>Simulation controls</summary><div id="balls"></div><button onclick="post('/api/sim/stroke',{})">Stroke</button><button onclick="post('/api/sim/feature',{feature_id:'precision_gate'})">Precision +25</button><button onclick="post('/api/sim/cup',{})">Cup</button></details></div><div class="card"><h3>Leaderboard</h3><div id="ranking" class="ranking"></div></div></section></main><script>
let current=null;async function state(){let r=await fetch('/api/state');if(!r.ok)return;current=await r.json();render()}function render(){let h=current.hole||{};document.querySelector('#hole').textContent='HOLE '+(h.number||'—');document.querySelector('#title').textContent=h.title||'PuttTrack';document.querySelector('#instruction').textContent=h.instructions||'';let c=current.cue||{state:'AVAILABLE',tone:'neutral'};document.querySelector('#status').textContent=c.state;document.querySelector('#lamp').className='lamp '+(c.tone||'');let ap=current.active_player;document.querySelector('#player').textContent=ap?ap.display_name+' · '+(current.ball_labels?.[ap.ball_id]||ap.ball_id):'Present your assigned ball';document.querySelector('#ranking').innerHTML=(current.ranking||[]).map(x=>`<div class=row><span>${x.rank}. ${x.display_name}</span><strong>${x.points} pts</strong></div>`).join('');document.querySelector('#balls').innerHTML=Object.entries(current.ball_labels||{}).map(([id,label])=>`<button onclick="post('/api/sim/tee',{ball_id:'${id}'})">${label}</button>`).join('')}async function post(path,body){let r=await fetch(path,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});let j=await r.json();document.querySelector('#toast').textContent=j.message||j.error||'';await state()}let es=new EventSource('/events');es.onmessage=e=>{let x=JSON.parse(e.data);document.querySelector('#toast').textContent=x.text||'';if(x.kind==='ball_detected'){let l=document.querySelector('#lamp');l.className='lamp amber';document.querySelector('#status').textContent='DETECTED / CHECKING'}setTimeout(state,180)};state();</script></body></html>'''

CHECKIN_HTML = r'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>PuttTrack Check-in</title><style>body{font-family:system-ui;background:#f4f5f8;margin:0;display:grid;place-items:center;min-height:100vh}.card{background:white;padding:30px;border-radius:22px;box-shadow:0 20px 60px #0002;width:min(520px,90vw)}input,button,textarea{box-sizing:border-box;width:100%;padding:14px;margin:7px 0;border-radius:10px;border:1px solid #ccd0d8}button{background:#111827;color:white;font-weight:700}</style></head><body><div class=card><h1>Start your game</h1><p>Guest play only needs a display name. Account linking is optional.</p><input id=booking placeholder="Booking code (optional)"><textarea id=players style="height:120px" placeholder="One player per line"></textarea><button onclick=start()>Assign smart balls</button><pre id=out></pre></div><script>async function start(){let players=document.querySelector('#players').value.split('\n').map(x=>x.trim()).filter(Boolean);let r=await fetch('/api/checkin',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({booking_code:document.querySelector('#booking').value||null,players})});let j=await r.json();document.querySelector('#out').textContent=JSON.stringify(j,null,2);if(r.ok)setTimeout(()=>location.href='/',1200)}</script></body></html>'''
